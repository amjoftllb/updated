...
const Message = ({ message, index, addMessage }: MessageProps) => {
  ...
  return (
    <div className="message-container">
      ...
      {typeof message_content === "string" ? (
        <TextMessage side={role} content={message_content} />
      ) : message_content &&
...
const TableGraph = ({ content }) => {
  const rows = content.graph_data.series.data.map((data, index) => ({
    id: index + 1,
    data,
  }));
  const columns = [
...
const TextMessage = ({ side, content }) => {
